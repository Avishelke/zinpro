...
class Insemination extends Component {
  ...
  componentDidMount = () => {
    ...
    db.transaction((tx) => {
      tx.executeSql(`select * from gestation_assessor WHERE evaluation_group_id=?`, [this.evaluationId], (tx, r) => {
        let row = r.rows.item(0);
        if (row) {
          this.setState({ ...row, ...{ is_sync: 0 } });
        } else {
          const {
            body_score_entry, body_score_entry_comments, fight_signs, fight_signs_comments, air, air_comments,
            temperature, temperature_comments, floor, floor_comments, rest_areas, rest_areas_comments,
            feed, feed_comments, management, management_comments, id
          } = this.state;

          let query = `INSERT INTO gestation_assessor(
            body_score_entry, body_score_entry_comments, fight_signs, fight_signs_comments, air, air_comments,
            temperature, temperature_comments, floor, floor_comments, rest_areas, rest_areas_comments,
            feed, feed_comments, management, management_comments, id, is_sync,DATE,evaluation_group_id
            ) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`;

          let params = [
            body_score_entry, body_score_entry_comments, fight_signs, fight_signs_comments, air, air_comments,
            temperature, temperature_comments, floor, floor_comments, rest_areas, rest_areas_comments,
            feed, feed_comments, management, management_comments, id, 0, this.date, this.evaluationId
          ];
  ...
  openImagePicker = () => {
    ...
    ImagePicker.openPicker({
      ...
      db.transaction((tx) => {
        tx.executeSql(`update gestation_assessor set ${index}= ? where id= ?`, [imaged, this.state.id], () => {
  ...
  fields = [
    ...
    { label: "FIGHT SIGNS", index: 'fight_signs' },
    { label: "BODY SCORE END", index: 'air' },
    { label: "TEMPERATURE", index: 'temperature' },
    { label: "FLOOR", index: 'floor' },
    { label: "REST AREAS", index: 'rest_areas' },
    { label: "FEEDING", index: 'feed' },
  ...
  saveAssessor() {
    let query = `UPDATE gestation_assessor SET 
    body_score_entry= ?,body_score_entry_comments= ?,
    fight_signs =?, fight_signs_comments=?, 
    air= ?,air_comments= ?,
    temperature =?, temperature_comments=?, 
    floor= ?,floor_comments= ?,
    rest_areas =?, rest_areas_comments=?,
    feed =?, feed_comments=?,
