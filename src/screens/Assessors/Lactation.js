...
class Insemination extends Component {
  ...
  componentDidMount = () => {
    ...
    db.transaction((tx) => {
      tx.executeSql(`select * from lactation_assessor WHERE evaluation_group_id=?`, [this.evaluationId], (tx, r) => {
        let row = r.rows.item(0);
        if (row) {
          this.setState({ ...row, ...{ is_sync: 0 } });
        } else {
          const {
            body_score_entry, body_score_entry_comments, body_score_middle, body_score_middle_comments,
            body_score_end, body_score_end_comments, temperature, temperature_comments, transition, transition_comments,
            feed, feed_comments, pds, pds_comments, creep_feed, creep_feed_comments, piglets, piglets_comments,
            hygiene, hygiene_comments, split_suckling_management, split_suckling_management_comments,
            cutting_edges, cutting_edges_comments, id
          } = this.state;

          let query = `INSERT INTO lactation_assessor(
            body_score_entry, body_score_entry_comments, body_score_middle, body_score_middle_comments,
            body_score_end, body_score_end_comments, temperature, temperature_comments, transition, transition_comments,
            feed, feed_comments, pds, pds_comments, creep_feed, creep_feed_comments, piglets, piglets_comments,
            hygiene, hygiene_comments, split_suckling_management, split_suckling_management_comments,
            cutting_edges, cutting_edges_comments, id, is_sync,DATE,evaluation_group_id
            ) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`;
          ...
          let params = [
            ...
            feed, feed_comments, pds, pds_comments, creep_feed, creep_feed_comments, piglets, piglets_comments,
            hygiene, hygiene_comments, split_suckling_management, split_suckling_management_comments,
            cutting_edges, cutting_edges_comments, id, 0, this.date, this.evaluationId
          ];
  ...
  openImagePicker = () => {
    ...
    ImagePicker.openPicker({
      ...
      db.transaction((tx) => {
        tx.executeSql(`update lactation_assessor set ${index}= ? where id= ?`, [imaged, this.state.id], () => {
  ...
  state = {
    ...
    body_score_middle: 0,
    body_score_middle_comments: '',
    body_score_middle_image: '',
    body_score_end: 0,
    body_score_end_comments: '',
    body_score_end_image: '',
    temperature: 0,
    ...
    hygiene_comments: '',
    hygiene_image: '',
    split_suckling_management: 0,
    split_suckling_management_comments: '',
    split_suckling_management_image: '',
    cutting_edges: 0,
    cutting_edges_comments: '',
    cutting_edges_image: '',
  ...
  fields = [
    ...
    { label: "WATER", index: 'body_score_middle' },
    { label: "BODY SCORE END", index: 'body_score_end' },
    { label: "TEMPERATURE", index: 'temperature' },
    { label: "TRANSITION STRATEGY", index: 'transition' },

    { label: "FEED", index: 'feed' },
    { label: "EARLY LACTATION", index: 'pds' },
    { label: "CREEP FEED", index: 'creep_feed' },
    { label: "PIGLETS", index: 'piglets' },
    { label: "HYGIENE", index: 'hygiene' },
    { label: "SPLIT SUCLIKNG MANAGEMENT", index: 'split_suckling_management' },
    { label: "FARROW CRATES", index: 'cutting_edges' }
  ];
  ...
  saveAssessor() {
    let query = `UPDATE lactation_assessor SET 
    body_score_entry= ?,body_score_entry_comments= ?,
    body_score_middle= ?,body_score_middle_comments= ?,
    body_score_end= ?,body_score_end_comments= ?,
    temperature= ?,temperature_comments= ?,
        transition= ?,transition_comments= ?,

    feed= ?,feed_comments= ?,pds= ?,pds_comments= ?,
    creep_feed= ?,creep_feed_comments= ?,
    piglets= ?,piglets_comments= ?,hygiene= ?,
    hygiene_comments= ?,split_suckling_management= ?,
    split_suckling_management_comments= ?,
    cutting_edges= ?,cutting_edges_comments= ?,
    is_active=? ,
    is_sync=?
    WHERE id= ?`;
    ...
    const {
      ...
      pds, pds_comments,
      creep_feed, creep_feed_comments,
      piglets, piglets_comments,
      hygiene, hygiene_comments,
      split_suckling_management, split_suckling_management_comments,
      cutting_edges, cutting_edges_comments,
      id
    } = this.state;
