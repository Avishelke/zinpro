...
class GiltList extends Component {
  ...
  componentDidMount() {
    // let query = `select printf('%s %s', e.first_name, e.last_name) as name, gc.date,gc.id, g.name as group_name, f.name as farm_name from gilt_break_even gc 
    ...
    let query = `select printf('%s %s', e.first_name, e.last_name) as name, gc.date,gc.id, f.name as farm_name, e.id as evaluatinId from gilt_break_even gc 
