...
class GiltList extends Component {
  ...
  componentDidMount() {
    // let query = `select printf('%s %s', e.first_name, e.last_name) as name, gc.date, gc.id, f.name as farm_name from gilt_assessor gc
    ...
    let query = `select printf('%s %s', e.first_name, e.last_name) as name, gc.date, gc.id, f.name as farm_name, e.id as evaluatinId from gilt_assessor gc
    inner join evaluations e on e.id = gc.evaluation_group_id
    inner join farms f on f.id = e.farm_id
    where gc.is_active=?  order by gc.id DESC`;
